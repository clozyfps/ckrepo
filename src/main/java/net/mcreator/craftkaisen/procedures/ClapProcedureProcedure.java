...
public class ClapProcedureProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		double currenty = 0;
		double currentz = 0;
		if ((entity.getPersistentData().getString("boogiepartner")).isEmpty()) {
		...
		} else if (!(entity.getPersistentData().getString("boogiepartner")).isEmpty()) {
			{
				...
				for (Entity entityiterator : _entfound) {
					if (!(entity == entityiterator)) {
						if ((entity.getPersistentData().getString("boogiepartner")).equals(entityiterator.getDisplayName().getString())) {
							if (world instanceof Level _level) {
								if (!_level.isClientSide()) {
									_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("craft_kaisen:clap")), SoundSource.NEUTRAL, 1, 1);
								} else {
