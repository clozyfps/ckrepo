					...
					if (distanceSq <= 1.0) {
						if (world.getBlockState(BlockPos.containing(x + xi, y + i, z + zi)).canOcclude()) {
							if (!world.getBlockState(BlockPos.containing(x + xi, y + i + 1, z + zi)).canOcclude()
									&& !((world.getBlockState(BlockPos.containing(x + xi, y + i + 1, z + zi))).getBlock() == (world.getBlockState(BlockPos.containing(entity.getX(), entity.getY(), entity.getZ()))).getBlock())) {
							...
							}
							{
								...
								for (Entity entityiterator : _entfound) {
									if (entityiterator instanceof FallingBlockEntity) {
										...
										entityiterator.setSecondsOnFire(8);
										entityiterator.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.ON_FIRE), immediatesourceentity, entity),
