...
public class DomainClashActiveProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (entity.getPersistentData().getBoolean("domainclash") == true) {
			...
			if (!(entity instanceof Player _plr20 && _plr20.containerMenu instanceof DomainClashMenu)) {
			...
			if (world instanceof Level _level) {
				if (!_level.isClientSide()) {
					_level.playSound(null, BlockPos.containing(x, y, z), ForgeRegistries.SOUND_EVENTS.getValue(new ResourceLocation("craft_kaisen:electric")), SoundSource.PLAYERS, 1, 1);
				} else {
