...
public class PiercingBloodOnEffectActiveTickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		double distance = 0;
		double countpiercing = 0;
		countpiercing = countpiercing + 1;
				...
				if (!projectileLevel.isClientSide()) {
					Projectile _entityToSpawn = new Object() {
						public Projectile getArrow(Level level, Entity shooter, float damage, int knockback) {
							AbstractArrow entityToSpawn = new PiercingBloodProjectileEntity(CraftKaisenModEntities.PIERCING_BLOOD_PROJECTILE.get(), level);
					...
					}.getArrow(projectileLevel, entity, (float) (10 + (entity.getCapability(CraftKaisenModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftKaisenModVariables.PlayerVariables())).currentOutput / 3), 0);
