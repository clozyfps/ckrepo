...
public class RenderVoidProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, PoseStack pose) {
		if (entity == null || pose == null)
			return;
		if ((entity.level.dimension()) == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("craft_kaisen:unlimited_void_domain")))) {
			...
			(pose).popPose();
			(pose).pushPose();
			(pose).mulPose(com.mojang.math.Axis.YN.rotationDegrees(15));
			(pose).mulPose(com.mojang.math.Axis.XP.rotationDegrees(-15));
			(pose).mulPose(com.mojang.math.Axis.ZN.rotationDegrees(2));
