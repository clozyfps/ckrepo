...
public class BlackFlashProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world, Entity entity, Entity sourceentity, double amount) {
		...
		if (sourceentity instanceof Player) {
			sourceentity.getPersistentData().putDouble("bf", (Mth.nextInt(RandomSource.create(), 1, 5000)));
			if (!(sourceentity instanceof LivingEntity _livEnt3 && _livEnt3.hasEffect(CraftKaisenModMobEffects.ZONE.get()))) {
				vanim = Mth.nextInt(RandomSource.create(), 1, 10000);
			} else if (sourceentity instanceof LivingEntity _livEnt5 && _livEnt5.hasEffect(CraftKaisenModMobEffects.ZONE.get())) {
			...
			if ((sourceentity.getCapability(CraftKaisenModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftKaisenModVariables.PlayerVariables())).currentCursedEnergy > 0) {
				if (sourceentity.getPersistentData().getDouble("bf") > (sourceentity.getCapability(CraftKaisenModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new CraftKaisenModVariables.PlayerVariables())).BlackFlashRarity) {
					...
					entity.hurt(new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.GENERIC), sourceentity), (float) (amount * 2.5));
